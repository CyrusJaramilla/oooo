...
const Documents = () => {
  ...
  const handleUpload = () => {
    ...
    const url = isEditing
      ? `http://localhost:3001/api/coed/${documents[currentIndex]?.id}`
      : 'http://localhost:3001/api/coed';
  ...
  const confirmDelete = () => {
    if (documentToDelete) {
      axios.delete(`http://localhost:3001/api/coed/${documentToDelete.id}`)
        .then(() => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3001/api/coed')
      .then((response) => {
  ...
  return (
    <div className="document-container">
      <div className="header-container">
        <h2>{isEditing ? 'Edit Document' : 'COED Extension Docs'}</h2>
