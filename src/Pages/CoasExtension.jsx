...
const Documents = () => {
  ...
  const handleUpload = () => {
    ...
    const url = isEditing
      ? `http://localhost:3001/api/cas/${documents[currentIndex]?.id}`
      : 'http://localhost:3001/api/cas';
  ...
  const confirmDelete = () => {
    if (documentToDelete) {
      axios.delete(`http://localhost:3001/api/cas/${documentToDelete.id}`)
        .then(() => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3001/api/cas')
      .then((response) => {
  ...
  return (
    <div className="document-container">
      <div className="header-container">
        <h2>{isEditing ? 'Edit Document' : 'CAS Extension Docs'}</h2>
