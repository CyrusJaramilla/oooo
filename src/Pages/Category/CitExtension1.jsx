...
const Document = () => {
  ...
  const categories = ['HINDI', 'KO', 'ALAM', 'ILALAGAY', 'HERE'];
  ...
  const handleUpload = () => {
    if (!title || !category || !description || !date || (!file && !isEditing)) {
    ...
    const url = isEditing
      ? `http://localhost:3001/api/cit/${documents[currentIndex]?.id}`
      : 'http://localhost:3001/api/cit';
  ...
  const confirmDelete = () => {
    if (documentToDelete) {
      axios.delete(`http://localhost:3001/api/cit/${documentToDelete.id}`)
        .then(() => {
  ...
  useEffect(() => {
    axios.get('http://localhost:3001/api/cit')
      .then((response) => {
    ...
    <>
      ...
      <div className="document-container">
        <div className="header-container">
          <h2>{isEditing ? 'Edit Document' : 'EWAN Extension Docs'}</h2>
          <select
            value={category}
